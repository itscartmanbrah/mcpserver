...
/**
 * Auto-generated endpoint tester for: IsItemImageNewerBySKU
 *
 ...
 *
 * Usage: ?sku=...&imageindex=...&comparedate=...
 *
 * Notes:
...
try {
  ...
  2 => 'comparedate=...',
    ...
    // Call SOAP method
    $response = $soapClient->IsItemImageNewerBySKU($payload);

    // Typical SOAP result property naming: <MethodName>Result
    $resultProp = 'IsItemImageNewerBySKUResult';
    $result = $response->{$resultProp} ?? $response;

    echo "IsItemImageNewerBySKU SUCCESS\n\n";
    print_r($result);

} catch (SoapFault $fault) {
    http_response_code(502);

    echo "IsItemImageNewerBySKU FAILED (SoapFault)\n";
