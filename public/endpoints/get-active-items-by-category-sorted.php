...
/**
 * Auto-generated endpoint tester for: GetActiveItemsByCategorySorted
 *
 * - AuthenticationInfo (AuthInfo)
 * - CategoryID (int)
 * - SortBy (ArrayOfSortParam)
 *
 * Usage: ?categoryid=...&sortby=...
 *
 * Notes:
...
try {
  ...
  1 => 'sortby=...',
    ...
    // Call SOAP method
    $response = $soapClient->GetActiveItemsByCategorySorted($payload);

    // Typical SOAP result property naming: <MethodName>Result
    $resultProp = 'GetActiveItemsByCategorySortedResult';
    $result = $response->{$resultProp} ?? $response;

    echo "GetActiveItemsByCategorySorted SUCCESS\n\n";
    print_r($result);

} catch (SoapFault $fault) {
    http_response_code(502);

    echo "GetActiveItemsByCategorySorted FAILED (SoapFault)\n";
