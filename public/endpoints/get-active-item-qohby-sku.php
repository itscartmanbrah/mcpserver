...
/**
 * Auto-generated endpoint tester for: GetActiveItemQOHBySKU
 *
...
try {
  ...
  0 => 'sku=...',
    ...
    // Call SOAP method
    $response = $soapClient->GetActiveItemQOHBySKU($payload);

    // Typical SOAP result property naming: <MethodName>Result
    $resultProp = 'GetActiveItemQOHBySKUResult';
    $result = $response->{$resultProp} ?? $response;

    echo "GetActiveItemQOHBySKU SUCCESS\n\n";
    print_r($result);

} catch (SoapFault $fault) {
    http_response_code(502);

    echo "GetActiveItemQOHBySKU FAILED (SoapFault)\n";
