...
/**
 * Auto-generated endpoint tester for: GetItemImageBySKU
 *
...
try {
  ...
  1 => 'imageindex=...',
    ...
    // Call SOAP method
    $response = $soapClient->GetItemImageBySKU($payload);

    // Typical SOAP result property naming: <MethodName>Result
    $resultProp = 'GetItemImageBySKUResult';
    $result = $response->{$resultProp} ?? $response;

    echo "GetItemImageBySKU SUCCESS\n\n";
    print_r($result);

} catch (SoapFault $fault) {
    http_response_code(502);

    echo "GetItemImageBySKU FAILED (SoapFault)\n";
