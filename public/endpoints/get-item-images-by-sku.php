...
/**
 * Auto-generated endpoint tester for: GetItemImagesBySKU
 *
...
try {
  ...
  0 => 'sku=...',
    ...
    // Call SOAP method
    $response = $soapClient->GetItemImagesBySKU($payload);

    // Typical SOAP result property naming: <MethodName>Result
    $resultProp = 'GetItemImagesBySKUResult';
    $result = $response->{$resultProp} ?? $response;

    echo "GetItemImagesBySKU SUCCESS\n\n";
    print_r($result);

} catch (SoapFault $fault) {
    http_response_code(502);

    echo "GetItemImagesBySKU FAILED (SoapFault)\n";
