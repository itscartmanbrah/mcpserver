...
try {
    ...
    $aiBrandCol = $itemCols['brandid'] ?? 'BrandID';

    // Price estimate: retail_price > current_price > price
    $priceExpr = "COALESCE(ai.RetailPrice, ai.CurrentPrice, ai.Price, 0)";

    // Brand id normalization: NULL/'' => 'NO_BRAND'
    $brandIdExpr = "COALESCE(NULLIF(ai.`{$aiBrandCol}`,''), 'NO_BRAND')";

    // Brand mapping (correct key: eweb_brands.BrandID)
    $brandJoinSql = "";
    $brandNameSelect = "
      CASE
        WHEN {$brandIdExpr} = 'NO_BRAND' THEN 'No brand / Unbranded'
        ELSE NULL
      END AS brand_name
    ";

    if (ai_table_exists($pdo, 'eweb_brands')) {
        $brandCols = ai_table_columns($pdo, 'eweb_brands');

        // use exact columns from your DESCRIBE output
        $bBrandId = $brandCols['brandid'] ?? 'BrandID';
        $bName    = $brandCols['name'] ?? 'Name';

        $brandJoinSql = "
          LEFT JOIN eweb_brands b
            ON {$brandIdExpr} <> 'NO_BRAND'
           AND " . ai_sku_join_on("b.`{$bBrandId}`", $brandIdExpr) . "
        ";
    ...
    ai_json_out([
        ...
        'brands' => $rows,
        'note' => 'Sales are inferred from negative inventory deltas (delta < 0). Estimated value uses COALESCE(RetailPrice, CurrentPrice, Price) from current ActiveItem data and may include adjustments. Items with no BrandID are grouped under brand_id=NO_BRAND.',
    ]);

} catch (Throwable $e) {
    error_log('[sales-by-brand] ERROR: ' . $e->getMessage());
